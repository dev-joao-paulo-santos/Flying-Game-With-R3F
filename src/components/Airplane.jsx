...
import shotSound from '/sounds/shot-mg.wav';
...
const Airplane = () => {
  ...
  useFrame((_, delta) => {
    ...
    ref.current.rotation.z = THREE.MathUtils.lerp(ref.current.rotation.z, roll, 0.1);
  ...
  const shoot = () => {
    ...
    bullet.position.copy(ref.current.position);
    bullet.position.addScaledVector(ref.current.getWorldDirection(new THREE.Vector3()), 2);

    //Define a direção da bala
    bullet.userData.direction = new THREE.Vector3();
    bullet.userData.direction.copy(ref.current.getWorldDirection(new THREE.Vector3()));
